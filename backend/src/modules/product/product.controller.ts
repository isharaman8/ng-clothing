...
import { PRODUCT_GET_PROJECTION, REVIEW_USER_PROJECTION } from 'src/constants/constants';
...
export class ProductController {
  ...
  async getProducts(@Query() query: any, @Res() response: CResponse) {
    ...
    products = await this.productService.getAllProducts(parsedQuery, PRODUCT_GET_PROJECTION);
  ...
  async createProductReview(@Body('review') _review: CreateOrUpdateProductReviewDto, @Res() response: CResponse) {
    ...
    let createdReview: any = await this.reviewService.createOrUpdateReview(review, oldReview);
